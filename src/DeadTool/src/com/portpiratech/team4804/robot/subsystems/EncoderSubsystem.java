...
public class EncoderSubsystem extends PIDSubsystem {
   ...
   //constants
	...
	double angleOffset = 30; //degrees; need to measure this value
  ...
  //Basic Encoder Motor speed/position methods
    ...
    public void move(XboxController xbox) {
    	setMotor(xbox.getLeftStickYAxis()*SmartDashboard.getNumber("Encoder Multiplier"));
    }
  ...
  //Position calculation/movement/math methods
   ...
   //PID methods
	...
	protected double returnPIDInput() {
		double distance = Robot.vision.launchAngle(Robot.vision.distanceFeet);
		double distanceX = distance * Math.cos(Robot.cannonEncoderMotor.get()*DEGREES_PER_PULSE - angleOffset);
		double targetAngle = launchAngle(distanceX) + angleOffset;
